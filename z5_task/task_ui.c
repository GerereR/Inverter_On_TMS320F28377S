...
static Uint16 OLED_FrameBuf[OLED_WIDTH_COLUMNS * OLED_HEIGHT_PAGES];
...
#define UI_DISPLAY_SCREEN_CNT 2U
#define UI_INIT_RETRY_LIM     2U  /* 1.5 s * 2 = 3 s */
#define UI_DISPLAY_PERIOD_TICK 5U   /* Task_UI每200ms运行一次,每1s轮换页面 */
...
/* Show the supervinsulry state and the two protection indicators together. */
                                      ...
                                      Uint16 tzFault)
{
    ...
    for(idx = position; idx < OLED_LINE_CHARS; idx++)
    {
        text[idx] = ' ';
    }
    text[OLED_LINE_CHARS] = '\0';
...
void Task_UI(void)
{
    ...
    {
        UI_InitRetryDivider++;
        if(UI_InitRetryDivider >= UI_INIT_RETRY_LIM)
        {
            UI_InitRetryDivider = 0U;
            Task_UI_Init();
        }
    ...
    /* Task_Measu和Task_UI同属合作式主循环,测量快照读取期间不会被改写。 */
    gridVoltRms = gMachineData.realRms.gridVolt;
    inductCurrRms = gMachineData.realRms.inductCurr;
    pv1Volt = gMachineData.realAvg.pv1Volt;
    pv1Curr = gMachineData.realAvg.pv1Curr;
    pv2Volt = gMachineData.realAvg.pv2Volt;
    pv2Curr = gMachineData.realAvg.pv2Curr;
    ecapFreq = (float)gMachineData.ecapFreqCent * 0.01f;
    pllFreq = (float)gMachineData.pllFreqCent * 0.01f;
    inductCurrAmp = gBusCtrlData.currAmpRef;
    boost1Duty = gBusCtrlData.boost1Duty;
    boost2Duty = gBusCtrlData.boost2Duty;
    pllLocked = ((gSysProblem.recovFault & RECOV_PLL_FAULT) == 0UL) ? 1U : 0U;
    tzFault = (gSysProblem.recovFault & RECOV_TZ_FAULT) != 0UL;

    /* 两组页面轮换显示, 每组都覆盖相同的页面, 不会残留上一组内容。 */
    if(UI_DisplayScreen == 0U)
    {
        OLED_WriteLine(0U, "MEASUMENTS");
        OLED_WriteFloat1Line(1U, "GRID RMS", gridVoltRms, "V");
        OLED_WriteFloat1Line(2U, "IND RMS", inductCurrRms, "A");
        OLED_WriteFloat1Line(3U, "PV1 VOLT", pv1Volt, "V");
        OLED_WriteFloat1Line(4U, "PV1 CURR", pv1Curr, "A");
        OLED_WriteFloat1Line(5U, "PV2 VOLT", pv2Volt, "V");
        OLED_WriteFloat1Line(6U, "PV2 CURR", pv2Curr, "A");
        OLED_WriteFloat1Line(7U, "DC BUS", gMachineData.realAvg.dcBusVolt, "V");
    }
    else
    {
        OLED_WriteLine(0U, "CONTROL STATUS");
        OLED_WriteFloat1Line(1U, "INV VOLT", gMachineData.realAvg.invertVolt, "V");
        OLED_WriteFloat1Line(2U, "ECAP FREQ", ecapFreq, "HZ");
        OLED_WriteFloat1Line(3U, "PLL FREQ", pllFreq, "HZ");
        OLED_WriteFloat1Line(4U, "IREF AMP", inductCurrAmp, "PU");
    ...
    UI_DisplayScreen++;
    if(UI_DisplayScreen >= UI_DISPLAY_SCREEN_CNT)
    {
        UI_DisplayScreen = 0U;
    }
...
static void OLED_Clear(void)
{
    memset(OLED_FrameBuf, 0, sizeof(OLED_FrameBuf));
...
static void OLED_WriteChar(char ch)
{
    ...
    if(ch == ' ')
    {
        fontIdx = OLED_FONT_IDX_SPACE;
    }
    else if((ch >= '0') && (ch <= '9'))
    {
        fontIdx = (Uint16)(OLED_FONT_IDX_DIGIT + (Uint16)(ch - '0'));
    }
    else if((ch >= 'A') && (ch <= 'Z'))
    {
        fontIdx = (Uint16)(OLED_FONT_IDX_ALPHA + (Uint16)(ch - 'A'));
    }
    else if((ch >= 'a') && (ch <= 'z'))
    {
        fontIdx = (Uint16)(OLED_FONT_IDX_LOWER + (Uint16)(ch - 'a'));
    }
    else
    {
        switch(ch)
        {
            case '.': fontIdx = (Uint16)(OLED_FONT_IDX_SYMBOL + 0U); break;
            case ':': fontIdx = (Uint16)(OLED_FONT_IDX_SYMBOL + 1U); break;
            case '?': fontIdx = (Uint16)(OLED_FONT_IDX_SYMBOL + 2U); break;
            case '!': fontIdx = (Uint16)(OLED_FONT_IDX_SYMBOL + 3U); break;
            case '(': fontIdx = (Uint16)(OLED_FONT_IDX_SYMBOL + 4U); break;
            case ')': fontIdx = (Uint16)(OLED_FONT_IDX_SYMBOL + 5U); break;
            case '-': fontIdx = (Uint16)(OLED_FONT_IDX_SYMBOL + 6U); break;
            case '_': fontIdx = (Uint16)(OLED_FONT_IDX_SYMBOL + 7U); break;
            case '~': fontIdx = (Uint16)(OLED_FONT_IDX_SYMBOL + 8U); break;
            default:  fontIdx = OLED_FONT_INDEX_QUESTION; break;
        }
    }

    bufIdx = (OLED_CursorPage * OLED_WIDTH_COLUMNS) + OLED_CursorColumn;
    for(column = 0U; column < 5U; column++)
    {
        OLED_FrameBuf[bufIdx + column] = OLED_Font5x7[fontIdx][column];
    }
    OLED_FrameBuf[bufIdx + 5U] = 0U;
...
static Uint16 OLED_RefreshPage(Uint16 page)
{
    ...
    {
        tx[column + 1U] = OLED_FrameBuf[(page * OLED_WIDTH_COLUMNS) + column];
    }
...
static Uint16 OLED_RefreshDirty(Uint16 maxPages)
{
    ...
    for(pageBias = 0U; pageBias < OLED_HEIGHT_PAGES; pageBias++)
    {
        page = scanStart + pageBias;
        if(page >= OLED_HEIGHT_PAGES)
        {
...
static void OLED_WriteFloat1Line(Uint16 page, const char *label, float value, const char *unit)
{
    ...
    for(divinsulr = 10000L; divinsulr > 0L; divinsulr /= 10L)
    {
        Uint16 digit = (Uint16)((integerPart / divinsulr) % 10L);
        if((digit != 0U) || (started != 0U) || (divinsulr == 1L))
        {
