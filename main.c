...
int main(void)
{
    ...
    while(1)
    {
        ...
        if(schedulerFlags & TASK_STATE_FLAG)       // 10 ms
        {
            Scheduler_ClearFlags(TASK_STATE_FLAG);
            Task_State();
        }

        if(schedulerFlags & TASK_MEASURE_FLAG)     // 20 ms
        {
            Scheduler_ClearFlags(TASK_MEASURE_FLAG);
            Task_Measure();
        }

        if(schedulerFlags & TASK_GRID_FLAG)        // 50 ms
        {
        ...
        if(schedulerFlags & TASK_POWER_FLAG)       // 500 ms
        {
